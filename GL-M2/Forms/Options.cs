        ...
        private void nP_ValueChanged(object sender, EventArgs e)
        {
            ...
            switch(numeric.Name)
            {
                case "npCircle_radius":
                    Properties.Settings.Default.circle_radius = (int)npCircle_radius.Value;
                    break;
                case "npTriangle_length":
                    Properties.Settings.Default.triangle_length = (int)npTriangle_length.Value;
                    break;
                case "npToggle_time":
                    Properties.Settings.Default.toggle_time = (int)npToggle_time.Value;
                    break;
                case "npTime_process":
                    Properties.Settings.Default.time_process = (int)npTime_process.Value;
                    break;
                case "npPercent_check":
                    Properties.Settings.Default.percent_check = (int)npPercent_check.Value;
                    break;
            }
            Properties.Settings.Default.Save();
        }
